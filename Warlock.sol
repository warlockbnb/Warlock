...
contract Warlock is ERC20, Ownable {
    ...
    WARLOCKDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = 0xeDeFBaE0A5031D8149F071Ee5FDc537B75cAAC1E;
    ...
    constructor() public ERC20("Warlock", "WRLK") {

    	dividendTracker = new WARLOCKDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "WARLOCK: The dividend tracker already has that address");

        WARLOCKDividendTracker newDividendTracker = WARLOCKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "WARLOCK: The new dividend tracker must be owned by the WARLOCK token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WARLOCK: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WARLOCK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WARLOCK: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WARLOCK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WARLOCK: Cannot update gasForProcessing to same value");
...
contract WARLOCKDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("WARLOCK_Dividen_Tracker", "WARLOCK_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 400000 * (10**9);
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "WARLOCK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "WARLOCK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WARLOCK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "WARLOCK_Dividend_Tracker: claimWait must be updated to between 20 mins and 24 hours");
        require(newClaimWait != claimWait, "WARLOCK_Dividend_Tracker: Cannot update claimWait to same value");
